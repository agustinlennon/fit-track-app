...
// Esta línea es la clave: lee las credenciales que pondremos en Netlify.
const firebaseConfig = JSON.parse(import.meta.env.VITE_FIREBASE_CONFIG);
// El appId lo obtenemos de la configuración que cargamos.
...
const Card = ({ children, className = '' }) => (
  <div
    className={`bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-md transition-all duration-300 ${className}`}
  >
    {children}
  </div>
);
...
const Button = ({
  ...
  const baseClasses =
    'px-4 py-2 rounded-lg font-semibold flex items-center justify-center gap-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-900 disabled:opacity-50 disabled:cursor-not-allowed';
  const variants = {
...
const Dashboard = ({
  ...
  const totals = useMemo(() => {
    return todaysLog.loggedFoods.reduce(
      (acc, food) => {
    ...
    <div>
      ...
      <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
        <div
          ...
          style={{ width: `${Math.min(getProgress(current, goal), 100)}%` }}
        ></div>
      </div>
    </div>
  );
  ...
  const getMotivationalMessage = () => {
    ...
    if (remainingProtein > 40)
      return {
        message: `¡Aún te faltan ${Math.round(
          remainingProtein
        )}g de proteína! Un batido o pollo podría ayudar.`,
        icon: <Flame className="text-orange-400" />,
      };
    if (remainingCarbs > 90)
      return {
        message: `¡Necesitas energía! Te faltan ${Math.round(
          remainingCarbs
        )}g de carbohidratos. ¿Qué tal avena o arroz?`,
        icon: <Dumbbell className="text-blue-400" />,
      };
    if (totals.calories > 0 && totals.calories < goals.calories / 2)
      return {
        message:
          '¡Buen comienzo! Sigue registrando para alcanzar tus metas de hoy.',
        icon: <CheckCircle className="text-green-400" />,
      };
    return {
      message:
        '¡Listo para romperla! Registra tu primera comida o entrenamiento.',
      icon: <Utensils className="text-gray-400" />,
    };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <HabitsTracker
            dailyLog={todaysLog}
            handleUpdateHabit={(habit, value) =>
              handleLogFood(today, { [habit]: value }, true)
            }
          />
        </Card>
      </div>
...
const FoodLogger = ({
  ...
  const getFoodsForMeal = (meal) =>
    todaysLog.loggedFoods.filter((f) => f.meal === meal);
  const removeFood = (foodToRemove) =>
    handleLogFood(today, { loggedFoods: arrayRemove(foodToRemove) }, true);
        ...
        {Object.entries(meals).map(([key, name]) => (
          <Card key={key}>
            ...
            <ul className="space-y-2">
              {getFoodsForMeal(key).length > 0 ? (
                getFoodsForMeal(key).map((food, index) => (
                  <li
                    key={index}
  ...
  const handleSubmit = (e) => {
    ...
    if (foodData) {
      ...
      const logEntry = {
        ...
        meal: mealType,
        calories: foodData.calories_per_100g * ratio,
        protein: foodData.protein_per_100g * ratio,
        carbs: foodData.carbs_per_100g * ratio,
        fat: foodData.fat_per_100g * ratio,
      };
...
const WorkoutPlanner = ({ userData, handleUpdateSchedule, handleGoBack }) => {
  ...
  const handleChange = (day, value) =>
    handleUpdateSchedule({ ...currentSchedule, [day]: value });
        ...
        <div className="space-y-4">
          {days.map((day) => (
            ...
            >
              ...
              <select
                value={currentSchedule[day] || 'Descanso'}
      ...
      {habits.map((habit) => (
        <div key={habit.id} className="flex items-center justify-between">
          ...
          {habit.id === 'morningRoutine' ? (
            <input
              type="checkbox"
              checked={dailyLog[habit.id] || false}
              onChange={(e) => handleUpdateHabit(habit.id, e.target.checked)}
              className="w-6 h-6 rounded text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
            />
          ) : (
            <input
              type="number"
              value={dailyLog[habit.id] || 0}
              onChange={(e) =>
                handleUpdateHabit(habit.id, Number(e.target.value))
              }
              className="w-20 p-1 text-center bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg"
            />
          )}
        </div>
      ))}
      ...
      <Card className="mb-6">
        ...
        >
          <input
            ...
            placeholder="Nombre"
            className="p-2 bg-gray-100 dark:bg-gray-700 border rounded col-span-2 md:col-span-1"
          />
          <input
            value={calories}
            ...
            className="p-2 bg-gray-100 dark:bg-gray-700 border rounded"
          />
          <input
            value={protein}
...
const AppSettings = ({ userData, handleUpdateGoals, handleGoBack }) => {
  ...
  const [goals, setGoals] = useState(
    userData.goals || { calories: 2500, protein: 180, carbs: 250, fat: 70 }
  );
  const handleChange = (e) =>
    setGoals({ ...goals, [e.target.name]: parseFloat(e.target.value) });
  const handleSubmit = (e) => {
    e.preventDefault();
    handleUpdateGoals(goals);
    alert('¡Objetivos actualizados!');
  };
    ...
    <div>
      ...
      <Card>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium">Calorías (kcal)</label>
            <input
              type="number"
              name="calories"
          ...
          <div>
            <label className="block text-sm font-medium">Proteínas (g)</label>
            <input
              type="number"
              name="protein"
              value={goals.protein}
          ...
          <div>
            <label className="block text-sm font-medium">Grasas (g)</label>
            <input
              type="number"
              name="fat"
              value={goals.fat}
...
const AiWorkoutGeneratorView = ({ userData, handleGoBack }) => {
  ...
  if (!userData) {
    return (
      <Card>
        <p>Cargando Asistente de Rutinas...</p>
      </Card>
    );
  }
  ...
  const getWorkoutSuggestion = async () => {
          ...
          routine: {
            ...
            items: {
              ...
              properties: {
                ...
                weight: {
                  type: 'STRING',
                  description: "Peso a utilizar (ej. '6 kg', 'Peso Corporal').",
                },
                videoSearchQuery: {
                  type: 'STRING',
                  description: 'Término de búsqueda en español para YouTube.',
                },
              },
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(`La llamada a la API falló: ${response.status}`);
      ...
      if (result.candidates?.[0]?.content?.parts?.[0]?.text) {
        ...
        if (!parsedJson.routine || parsedJson.routine.length === 0) {
          setError(
            'La IA no pudo generar una rutina esta vez. Inténtalo de nuevo.'
          );
        }
      } else {
        setError(
          'No se pudo obtener una rutina. La respuesta de la IA estaba vacía.'
        );
      }
    } catch (err) {
      console.error(
        'Error al llamar a la API de Gemini o al procesar la respuesta:',
        err
      );
      setError(
        `Ocurrió un error al contactar al asistente de IA. Por favor, revisa la consola para más detalles.`
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <Card>
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Presiona el botón para que la IA genere tu rutina de hoy, basada en
            tu plan semanal y tu equipamiento.
          </p>
          <Button
      ...
      {isLoading && (
        <div className="mt-6 text-center p-4">
          <p className="animate-pulse text-lg">
            El entrenador IA está preparando tu sesión...
          </p>
        </div>
      )}
      {error && (
        <div className="mt-6 p-4 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 rounded-lg">
          {error}
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      // Fallback a modo de prueba si la configuración de Firebase falla
      setUserData(MOCK_USER_DATA);
      setDailyLog(MOCK_DAILY_LOG);
      setWeightHistory(MOCK_WEIGHT_HISTORY);
      setFoodDatabase(MOCK_FOOD_DATABASE);
      setUserId('test-user-fallback');
      setIsAuthReady(true);
    }
  }, []);

  const handleAddFoodToDb = useCallback(
    async (foodData) => {
      if (!db || !userId) return;
      await addDoc(
        collection(db, `artifacts/${appId}/users/${userId}/foodDatabase`),
        foodData
      );
    },
    [db, userId]
  ...
  useEffect(() => {
    if (isAuthReady && db && userId) {
      ...
      const unsubUser = onSnapshot(
        doc(db, `${userDocPath}/profile/data`),
        (doc) => {
          if (doc.exists()) {
      ...
      const unsubLogs = onSnapshot(
        collection(db, `${userDocPath}/dailyLogs`),
        (snap) =>
          setDailyLog(
            snap.docs.reduce(
      ...
      const unsubWeight = onSnapshot(
        collection(db, `${userDocPath}/weightHistory`),
        (snap) =>
          setWeightHistory(
            snap.docs
      ...
      const unsubFood = onSnapshot(
        collection(db, `${userDocPath}/foodDatabase`),
        (snap) => {
          const foods = snap.docs.map((d) => ({ ...d.data(), id: d.id }));
          setFoodDatabase(foods);
          if (foods.length === 0) {
            [
              { name: 'Pechuga de Pollo', c: 165, p: 31, h: 0, g: 3.6 },
              { name: 'Arroz Blanco Cocido', c: 130, p: 2.7, h: 28, g: 0.3 },
              { name: 'Huevo Entero', c: 155, p: 13, h: 1.1, g: 11 },
              { name: 'Avena en Hojuelas', c: 389, p: 16.9, h: 66.3, g: 6.9 },
            ].forEach((f) =>
              handleAddFoodToDb({
                name: f.name,
                calories_per_100g: f.c,
  ...
  }, [isAuthReady, db, userId, handleAddFoodToDb]);
  ...
  const handleLogFood = useCallback(
    async (date, data, merge = false) => {
      if (!db || !userId) return;
      await setDoc(
        doc(db, `artifacts/${appId}/users/${userId}/dailyLogs`, date),
        data,
        { merge: merge }
      );
    },
  ...
  const handleAddWeight = async (weight) => {
    if (!db || !userId) return;
    await addDoc(
      collection(db, `artifacts/${appId}/users/${userId}/weightHistory`),
  ...
  const handleDeleteFood = async (foodId) => {
    if (!db || !userId) return;
    await deleteDoc(
      doc(db, `artifacts/${appId}/users/${userId}/foodDatabase`, foodId)
    );
  };
  const handleAddFood = async (foodData) => {
    await handleAddFoodToDb(foodData);
  };
  ...
  if (!isAuthReady) {
    return (
      <div className="flex items-center justify-center h-screen bg-gray-100 dark:bg-gray-900">
        <div className="text-center">
          <Flame className="mx-auto h-12 w-12 text-blue-600 animate-pulse" />
  ...
  const NavItem = ({ icon: Icon, label, viewName }) => (
    <button
      ...
      className={`flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 sm:gap-3 p-2 sm:px-4 rounded-lg w-full text-left transition-colors ${
        view === viewName
          ...
          <div className="flex flex-row sm:flex-col justify-around sm:justify-start sm:gap-2 h-full">
            ...
            <div className="mt-auto hidden sm:block">
              ...
              <button
                ...
                className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 mt-2"
              >
